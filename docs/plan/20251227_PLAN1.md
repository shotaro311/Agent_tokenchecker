...
**現在地**: Phase 3 - 完了条件のチェック更新
**次のアクション**: ユーザー側でUI上の表示を軽く確認 → リリース作業へ
**ブロッカー**: なし
...
- [x] 既存テストの期待値を言語に合わせて調整、必要なら新規テスト追加

### Phase 3: テスト/動作確認/ドキュメント同期
- [x] ドキュメントを日本語化（README/CHANGELOG/docs配下/サイトHTML）
- [x] `swift test` を実行（1件スキップ: Claude PTY タイムアウト）
- [x] `pnpm check` を実行
- [x] `./Scripts/compile_and_run.sh --debug-lldb` を実行し、アプリ起動を確認
- [x] `./Scripts/compile_and_run.sh` を実行し、Release署名でアプリ起動を確認
...
### 機能要件
- [x] 設定画面で日本語/Englishが切替できる。
- [x] 初期言語が日本語になっている。
- [x] UI/表示文言が選択言語に応じて切替される。

### 品質要件
- [x] `swift test` が成功する。
- [x] `pnpm check` が成功する。
- [x] `./Scripts/compile_and_run.sh` 実行後、アプリが起動して切替が確認できる。
...
- `swift test`: 成功（272 tests passed）
- `pnpm check`: 成功（swiftformat --lint / swiftlint --strict）
- `./Scripts/compile_and_run.sh`: Release署名で起動確認済み（Developer ID Applicationの署名ID検出を修正）
- `xattr`: PATH上の別`xattr`で `-r` が無効な環境があったため、`Scripts/package_app.sh` は `/usr/bin/xattr` を明示して使用
